...
class WatchTextEmbeddingGenerator:
    ...
    def generate_watch_description(self, brand: str, model: str) -> str:
        """
        Generate a watch description focusing on vibe and design language.
        ...
        """
        prompt = f"""Give me a description for the {brand} {model} watch. Focus on the vibe and design language of this timepiece. 

Describe:
- The overall aesthetic and design philosophy
- The visual character and personality
- The mood and feeling it conveys
- The design elements that make it distinctive
- The target audience or lifestyle it represents

Do NOT describe:
- Specific technical specifications
- Strap or band details
- Exact measurements
- Price information
- Availability

Keep the description concise (2-3 sentences) and focused on the visual and emotional aspects of the design."""

        try:
            response = self.client.chat.completions.create(
                model=self.description_model,
                messages=[
                    {"role": "system", "content": "You are an expert watch designer and critic with deep knowledge of horological aesthetics and design language."},
            ...
            return f"A distinctive timepiece from {brand}, the {model} represents the brand's signature design approach with its unique aesthetic character."
    ...
    def process_watch_data(self, watch_data: List[Dict], output_dir: str = "output") -> Tuple[np.ndarray, List[Dict]]:
        """
        ...
        print(f"Processing {len(watch_data)} watches...")
        
        for i, watch in enumerate(tqdm(watch_data, desc="Generating descriptions and embeddings")):
            try:
                ...
                description = self.generate_watch_description(brand, model)
                ...
                if (i + 1) % 10 == 0:
                    self._save_intermediate_results(enhanced_watches, embeddings, output_path, i + 1)
...
def main():
    ...
    # Process watches
    embedding_matrix, enhanced_watches = generator.process_watch_data(watch_data)
    ...
    print(f"\n✅ Successfully generated text embeddings for {len(enhanced_watches)} watches")
